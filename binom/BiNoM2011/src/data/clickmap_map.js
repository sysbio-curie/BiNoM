...
function hide_google_dialog_perform()
{
    var found = false;
    $('div').each(function(i, elem) {
	// 2019-07-03: detect google dialog according to style (as it has no id): WARNING this is fragile as:
	//   1. it can change with new google apis version
	//   2. it could catch a navicell div (less probable)
        if ($(elem).css("font-weight") == 500 &&
	    $(elem).css("font-family") == "Roboto, sans-serif" &&
	    $(elem).css("position") == "absolute") {
	    console.log("hiding google dialog [?]");
	    $(elem).css("display", "none");
	}
    });
}

function hide_google_dialog() {
    for (var nn = 1; nn < 40; nn++) {
	setTimeout(function() {hide_google_dialog_perform();}, nn*100);
    }
}
