...
{
    navicell.mapdata.whenReady(module_name, function() {
...
var MODULE_REGEX = new RegExp(">MODULE:\\w+", "g"); // 2019-07-02: added for module score support
...
Mapdata.prototype = {
    ...
    id_modules: {}, // added for module scores: list of modules (MODULE: tag) for each id (may be empty)
    module_ids: {}, // added for module scores
    hugo_modules: {}, // added for module scores
    module_hugos: {}, // added for module scores
    class_list: {},
	...
	for (var id in info) {
	    ...
	    //console.log("INFO for " + id + " " + info_id);
	    ...
	    // 2019-07-02: module score support: filling id to modules map
	    if (this.id_modules[id]) {
		console.log("** warning ** " + id + " has already a module list");
	    }
	    this.id_modules[id] = [];
	    // skipping information after Modification:
	    var modif_idx = info_id.search("Modification:");
	    if (modif_idx > 0) {
		arr = info_id.substring(0, modif_idx).match(MODULE_REGEX);
	    } else {
		arr = info_id.match(MODULE_REGEX);
	    }
	    if (arr && arr.length) {
		//var positions = this.module_mapdata_by_id[module_name][id].positions;
		var hugo_list = this.module_mapdata_by_id[module_name][id].entity['hugo'];
		for (var nn = 0; nn < arr.length; ++nn) {
		    var tag = arr[nn].replace(TAG_CLEAN_REGEX, "");
		    var id_module_name = tag.split(":")[1];
		    this.id_modules[id].push(id_module_name);
		    this.module_ids[id_module_name] = 1;
		    // 2019-07-03: will be used to compute module scores when loading protein expression, mrna etc. data
		    if (hugo_list) {
			for (var jj in hugo_list) {
			    var hugo_name = hugo_list[jj];
			    if (!this.hugo_modules[hugo_name]) {
				this.hugo_modules[hugo_name] = [];
			    }
			    this.hugo_modules[hugo_name].push(id_module_name);
			    if (!this.module_hugos[id_module_name]) {
				this.module_hugos[id_module_name] = [];
			    }
			    this.module_hugos[id_module_name].push(hugo_name);
			}
		    }
		    //this.id_modules[id][id_module_name] = 1; // this map is probably not necessary
		    //if (!this.module_ids[id_module_name]) {
		    //this.module_ids[id_module_name] = [];
		    //}
		    //var module_ids = this.module_ids[id_module_name];
		    //for (var ll in positions) {
		    //var pos = positions[ll];
		    //if (pos.said) {
		    //module_ids.push(pos.said);
		    //}
		    //}
		}
	    }
	...
	//this.displayIdModuleMap(module_name);
    },

    // 2019-07-02: module score support
    displayIdModuleMap: function(module_name) {
	for (var id in this.id_modules) {
	    var msg = "ID " + id + " modules {";
	    var idx = 0;
	    for (var idx in this.id_modules[id]) {
		var id_module_name = this.id_modules[id][idx];
		if (idx != 0) {msg += ", ";}
		msg += id_module_name;
		idx++;
	    }
	...
	for (var hugo_name in this.hugo_modules) {
	    var msg = "HUGO " + hugo_name + " modules {";
	    var idx = 0;
	    for (var idx in this.hugo_modules[hugo_name]) {
		var id_module_name = this.hugo_modules[hugo_name][idx];
		if (idx != 0) {msg += ", ";}
		msg += id_module_name;
		idx++;
	    }
	...
	/*
	  for (var id_module_name in this.module_ids) {
	  var msg = "module " + id_module_name + " said {";
	  var idx = 0;
	  for (var idx in this.module_ids[id_module_name]) {
	  var said = this.module_ids[id_module_name][idx];
	  if (idx != 0) {msg += ", ";}
	  msg += said;
	  idx++;
	  }
...
Dataset.prototype = {
	...
	datatable.ready.then(function() {
	    console.log("datatable.ready " + datatable.error);
	    if (!datatable.error) {
		dataset.addDatatable(datatable);
		update_status_tables(); // 2019-07-02
		if (datatable.getModuleScores()) {
		    dataset.readDatatable("Module scores", datatable.name + " module scores", null, "@DATA\n" + datatable.getModuleScoreData(), win, async);
		}
	    }
	});
	return datatable;
	...
	console.log("add datatable: " + datatable.getId());
	...
	console.log("add datatable 2: " + datatable.name);
	this.datatables[datatable.name] = datatable;
	this.datatables_canon_name[datatable.canon_name] = datatable;
    },
	...
	for (var jj = 0; jj < navicell.module_names.length; ++jj) {
		...
		var hugo_module_map = gene.hugo_module_map;
		var entity_map_arr = hugo_module_map[module_name];
		if (!entity_map_arr) {
		    continue;
		}
...
DisplayContinuousConfig.prototype = {
	...
	*/
	...
	//console.log("setStepInfo: " + config + " " + tabname + " " + idx + " " + value + " " + color + " " + size + " " + shape);
	...
	//console.log("_getShape(idx=" + idx + ", tabname=" + tabname + ", value=" + value + ", ret=" + this.shapes[tabname]['shape'][idx] + ")");
	if (idx < 0) {
	    return undefined;
	}
	...
	//console.log("getValueByModifId: " + modif_id + " sample_name=" + sample_name + " value1=" + value + ", value2=" + this.getDatatableValue(config, tabname, value));
	return this.getDatatableValue(config, tabname, value);
    },
...
DisplayUnorderedDiscreteConfig.prototype = {
	...
	if (idx < this.colors[tabname][config].length) {
	    this.colors[tabname][config][idx] = color;
	    //console.log("setValueInfo shape etc. " + idx + " " + tabname + " -> " + shape + " " + color + " " + size + " " + config + " " + tabname);
	    this.sizes[tabname][config][idx] = size;
	    this.shapes[tabname][config][idx] = shape;
...
Datatable.prototype = {
	...
	var is_module_scores = this.isModuleScores();
	var samples_to_add = [];
	var genes_to_add = [];
	...
	for (var gene_nn = 0, gene_jj = 1; gene_jj < gene_length; ++gene_jj) {
	    ...
	    if (is_module_scores) {
		// gene_name is module:
		var id_module_name = gene_name;
		if (!navicell.mapdata.module_ids[id_module_name]) {
		    continue;
		}
		var gene_list = navicell.mapdata.module_hugos[id_module_name];
		if (gene_list) {
		    for (var kk in gene_list) {
			genes_to_add.push(gene_list[kk]); // attempt
		    }
		}
	    } else {
	...
	this.computeModuleScores(samples_to_add, genes_to_add);
	this.epilogue(win);
	ready.resolve(this);
	console.log("datatable loaded #1 " + this.error);
	dataset.syncModifs();
	console.log("datatable loaded #2 " + this.error);
    },
    ...
    computeModuleScores: function(samples_to_add, genes_to_add) {
	if (this.isModuleScores() || !this.biotype.isContinuous()) { // maybe more conditions?
	    return;
	}
	//console.log("samples_to_add " + samples_to_add);
	//console.log("genes_to_add " + genes_to_add);
	var module_scores = {};
	for (var gene_nn = 0; gene_nn < genes_to_add.length; gene_nn++) {
	    var gene_name = genes_to_add[gene_nn];
	    var module_list = navicell.mapdata.hugo_modules[gene_name];
	    if (module_list) {
		for (var nn in module_list) {
		    var id_module_name = module_list[nn];
		    if (!module_scores[id_module_name]) {
			module_scores[id_module_name] = {};
		    }
		    for (var sample_nn = 0; sample_nn < samples_to_add.length; sample_nn++) {
			var sample_name = samples_to_add[sample_nn];
			if (!module_scores[id_module_name][sample_name]) {
			    module_scores[id_module_name][sample_name] = [0., 0];
			}
			module_scores[id_module_name][sample_name][0] += parseFloat(this.data[gene_nn][sample_nn]);
			module_scores[id_module_name][sample_name][1]++;
		    }
		}
	    }
	}

	this.module_scores = module_scores;
	//this.displayModuleScores();
    },

    getModuleScores: function() {
	if (this.module_scores == null || mapSize(this.module_scores) == 0) {
	    return null;
	}
	return this.module_scores;
    },

    getModuleScoreData: function() {
	var module_scores = this.module_scores;
	if (!module_scores) {
	    return '';
	}
	var str = "MODULE";
	for (var id_module_name in module_scores) {
	    for (var sample_name in module_scores[id_module_name]) {
		str += "\t" + sample_name;
	    }
	    break;
	}
	...
	for (var id_module_name in module_scores) {
	    str += id_module_name;
	    for (var sample_name in module_scores[id_module_name]) {
		str += "\t" + (module_scores[id_module_name][sample_name][0]/module_scores[id_module_name][sample_name][1]);
	    }
	    str += "\n";
	}
	return str;
    ...
    displayModuleScores: function() {
	console.log(this.getModuleScoreData());
    },
    ...
    isModuleScores: function() {
	return this.biotype.isModuleScores();
    },
	...
	*/
	...
	var gene_list;
	var get_gene;
	if (this.isModuleScores()) {
	    gene_list = navicell.mapdata.id_modules[modif_id];
	    if (gene_list.length == 0) {
		return 0;
	    }
	    get_gene = function(gene_list, nn) {
		return gene_list[nn];
	    };
	} else {
	    ...
	    gene_list = info[0];
	    get_gene = function(gene_list, nn) {
		return gene_list[nn].name;
	    };
	}

	if (method == Group.DISCRETE_VALUE) {
	    return this.getValue(sample_name, get_gene(gene_list, 0)); // warning: only one gene is taken into account
	}
	if (gene_list.length == 1) {
	    return this.getValue(sample_name, get_gene(gene_list, 0))*1.;
	}
	if (!method) {
	...
	if (method == Group.CONTINUOUS_MEDIAN) {
	    var values = [];
	    for (var nn = 0; nn < gene_list.length; ++nn) {

		var gene_name = get_gene(gene_list, nn);
	...
	if (method == Group.CONTINUOUS_AVERAGE || method == Group.CONTINUOUS_ABS_AVERAGE) {
		...
		return total_value / gene_list.length;
	    }
	    return total_absvalue / gene_list.length;
	}
...
BiotypeType.prototype = {
    ...
    isModuleScores: function() {
	return this.type == navicell.MODULE_SCORES;
    },
...
Biotype.prototype = {
    ...
    isModuleScores: function() {
	return this.type.isModuleScores();
    },
...
function navicell_init() {
    ...
    _navicell.SET = 40;
    _navicell.MODULE_SCORES = 50; // 2019-07-02: module scores support
	...
	_navicell.biotype_factory.addBiotype(new Biotype("Mutation data", new BiotypeType(_navicell.MUTATION, _navicell.UNORDERED_DISCRETE)));
   	_navicell.biotype_factory.addBiotype(new Biotype("Module scores",  new BiotypeType(_navicell.MODULE_SCORES, _navicell.CONTINUOUS)));
