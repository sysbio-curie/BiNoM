...
public class PathConsistencyAnalyzerPathDialog extends JDialog {
		...
		significanceTestB.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				PathConsistencyAnalyzerBootstrapDialog bootstrap = new PathConsistencyAnalyzerBootstrapDialog(Cytoscape.getDesktop(), "Significance analysis", true, analyzer);
	    ...
	    public void makeActiveNodeNetwork() throws Exception{
	    	...
	    	if(selectedPathway>=0){
	    		...
	    		NetworkFactory.createNetwork(selectedNode.Id+"_"+listOfPath.size(), XGMML.convertGraphToXGMML(graph), Cytoscape.getCurrentNetworkView().getVisualStyle(), false, null);
	    	}
	    	}
	    ...
	    public void makeTargetNetwork() throws Exception{
	    	...
	    	if(Cytoscape.getCurrentNetwork()!=null){
		    	if(selectedPathway>=0){
		    		...
		    		NetworkFactory.createNetwork(((Node)analyzer.pathwayNodes.get(selectedPathway)).Id+"_"+paths.size(), XGMML.convertGraphToXGMML(graph), Cytoscape.getCurrentNetworkView().getVisualStyle(), false, null);
		    	}
	    	}
	    }
