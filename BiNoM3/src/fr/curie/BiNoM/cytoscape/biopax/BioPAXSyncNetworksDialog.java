...
public class BioPAXSyncNetworksDialog extends JDialog {
	...
	for (int n = 0; n < len; n++) {
	    ...
	    data.add(((CyNetwork)netwV.get(n)).getRow(((CyNetwork)netwV.get(n))).get(CyNetwork.NAME, String.class));
	    ind[n] = n;
	}
	...
	okB.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			if (ind.length > 0) {
				...
				Collection<CyNetworkView> networkViews = Launcher.getAdapter().getCyNetworkViewManager().getNetworkViews(wholeNetw);
				Iterator<CyNetworkView> networkViewIterator = networkViews.iterator();
				CyNetworkView wholeNetwView =null;
				while(networkViewIterator.hasNext())
					wholeNetwView = networkViewIterator.next();
				
				nullNetw.getRow(nullNetw).set(CyNetwork.NAME, "New");
				...
				NetworkOperation op = new SyncNetworkIntersection(netw, wholeNetw);
				op.eval(nullNetw, nullNetwView);
				Launcher.getAdapter().getVisualMappingManager().setVisualStyle(Launcher.getAdapter().getVisualMappingManager().getVisualStyle(wholeNetwView), nullNetwView);
			    ...
			    try{
			    	Launcher.getAdapter().getCyNetworkManager().destroyNetwork(wholeNetw);
			    }catch(Exception ex){}
			}
		    }
		    catch(Exception ex) {
	...
	private HashMap buildNodeMap(CyNetwork netw) {
	    java.util.Iterator i = netw.getNodeList().iterator();
	    HashMap map = new HashMap();
	    int l = 0;
	    while(i.hasNext()) {
		CyNode node = (CyNode)i.next();
		String uri_arr[] = BioPAXPropertyManager.getURIs(node);
		System.out.println(netw.getRow(node).get(CyNetwork.NAME, String.class) + " -> " + uri_arr.length);
		l += uri_arr.length;
	...
	private HashMap buildEdgeMap(CyNetwork netw) {
	    java.util.Iterator i = netw.getEdgeList().iterator();
	    HashMap map = new HashMap();
	    while(i.hasNext()) {
