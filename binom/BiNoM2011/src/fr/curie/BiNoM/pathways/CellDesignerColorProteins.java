...
public class CellDesignerColorProteins {
	...
	public static void colorProteins(String CDFileName, String featureTableName, String minValue, String maxValue){
		try{
			CellDesignerColorProteins cl = new CellDesignerColorProteins();
			...
			if((featureTableName!=null)&&((new File(featureTableName)).exists())){
				...
				for(int i=1;i<tabl.fnames.length;i++){
					...
					for(int k=0;k<sbml.getSbml().getModel().getAnnotation().getCelldesignerListOfSpeciesAliases().getCelldesignerSpeciesAliasArray().length;k++){
						CelldesignerSpeciesAliasDocument.CelldesignerSpeciesAlias csa = sbml.getSbml().getModel().getAnnotation().getCelldesignerListOfSpeciesAliases().getCelldesignerSpeciesAliasArray(k);
						//csa.getCelldesignerUsualView().getCelldesignerPaint().getColor().setStringValue("ffffffff"); // white color
						csa.getCelldesignerUsualView().getCelldesignerPaint().getColor().setStringValue("ffd3d3d3"); // light grey
					}

					// build colors
					Color cls[] = cl.getColorCode(tabl,i, false,false, minValue, maxValue);
					System.out.println("Processing "+tabl.fnames[i]+" ...");
					for(int j=0;j<cls.length;j++){
						if(j==1000*(int)(0.001f*(float)j))
							System.out.print(j+" ");
	...
	public Color[] getColorCode(SimpleTable tab, int colNum, boolean maxInterval, boolean logScale, String minValue, String maxValue){
		
		// adjust num values to min. and max set by the user
		boolean adjust = false;
		float minVal = 0f;
		float maxVal = 0f;
		if (!minValue.equals("") && !maxValue.equals("")) {
			adjust = true;
			minVal = Float.parseFloat(minValue);
			maxVal = Float.parseFloat(maxValue);
		}
...
//					System.out.println("CycD\t"+val+"\t"+"val1="+val1+"\t"+x+"\t"+minv+"\t"+maxv);
	...
	/**
	 * create green - white - red color from 0,1 float value
	 * @param x
	 * @return Color
	 */
	public static Color getSpektrumEB(float x) {
		int val;
		Color col = null;
		// green to white
		if (x < 0.5) {
			val = (int)  (255 * 2 * x);
			col = new Color(val, 255, val);
		}
		// white to red
		if (x > 0.5) {
			val = (int) (255 * 2 * (1-x));
			col = new Color(255, val, val);
		}
		return col;
	}
