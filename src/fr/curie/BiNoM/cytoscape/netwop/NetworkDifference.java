...
//    }
	...
	Iterator<CyNode> i = left.getNodeList().iterator();
		...
		while (i.hasNext()) {
		    ...
		    if (Launcher.findNodeWithName(right, id) == null){
		...
		while (ii.hasNext()) {
		    ...
		    String id = left.getRow(edge).get(CyNetwork.NAME, String.class);
		    //if (!right.containsEdge(edge))
		    if(Launcher.findEdgeWithName(right, id) == null)
	...
	while (i.hasNext()) {
	    CyNode node = (CyNode)i.next();
	    if (Launcher.findNodeWithName(right, left.getRow(node).get(CyNetwork.NAME, String.class)) == null){
	...
	while (i.hasNext()) {
	    ...
	    if (Launcher.findEdgeWithName(right, left.getRow(edge).get(CyNetwork.NAME, String.class)) == null){
	    //if (!right.containsEdge(edge)) {
