...
public class NestUtils {
	...
	public static ArrayList<CyNode> getNodeArray(CyNetwork network){
		ArrayList<CyNode> array=new ArrayList<CyNode>();
	...
	*/
	public static CyEdge connect2NodesFrom(CyNode src,CyNode tgt,CyEdge fromEdge,String idInterValue){
	...
	*/
	...
	public static void reportPosition(CyNetworkView fromView,CyNetworkView toView){
		for(CyNode node:getNodeList(toView.getModel())){
			View<CyNode> toNodeView=toView.getNodeView(node);
			View<CyNode> fromNodeView=fromView.getNodeView(node);
			if (fromNodeView!=null){
				toNodeView.setVisualProperty(BasicVisualLexicon.NODE_X_LOCATION,(Double.parseDouble(fromNodeView.getVisualProperty(BasicVisualLexicon.NODE_X_LOCATION).toString())));
				toNodeView.setVisualProperty(BasicVisualLexicon.NODE_Y_LOCATION,(Double.parseDouble(fromNodeView.getVisualProperty(BasicVisualLexicon.NODE_Y_LOCATION).toString())));
			}
		}
	}
	/**
	...
	public static TreeMap<String,CyNetwork> getNetworksMap(){
		...
		for(CyNetwork network:Launcher.getAdapter().getCyNetworkManager().getNetworkSet()) networks.put(network.getRow(network).get(CyNetwork.NAME, String.class),network);
		return networks;
	}
	/**
	...
	public static String selectOneNetwork(TreeMap<String,CyNetwork> networks,String title, String label){
		...
		String selected=(String)JOptionPane.showInputDialog (null, /*Cytoscape.getDesktop()*/label,title,
	...
	public static ArrayList<String> selectNetworks(TreeMap<String,CyNetwork> networks,String title, String label){
		...
		ListDialog listBox=new ListDialog(null ,/*Cytoscape.getDesktop(),*/title,label,netNames);
