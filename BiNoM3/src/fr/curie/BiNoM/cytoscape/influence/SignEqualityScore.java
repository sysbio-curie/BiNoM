	...
	boolean getSetData(){
		...
		for(String key:setNames.keySet()){
			...
			for(int n=0;n<wgs.nodes.size();n++){
				...
				Double d=Cytoscape.getNodeAttributes().getDoubleAttribute(wgs.nodes.get(n).getNetworkPointer().getRow(wgs.nodes.get(n)).get(CyNetwork.NAME, String.class),setNames.get(key));			
				if(d!=null){
					activIn[is][n]=d;
			...
			for(int n=0;n<wgs.nodes.size();n++){
				...
				Double d=Cytoscape.getNodeAttributes().getDoubleAttribute(wgs.nodes.get(n).getNetworkPointer().getRow(wgs.nodes.get(n)).get(CyNetwork.NAME, String.class),aimNames.get(key));			
	...
	void stateOkNo(int set){
		...
		if(OUT_LOG){
			for(int n=0;n<activAim[set].length;n++){
				if(activAim[set][n]!=0){
					txt.append(wgs.nodes.get(n).getNetworkPointer().getRow(wgs.nodes.get(n)).get(CyNetwork.NAME, String.class));
