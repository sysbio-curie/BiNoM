...
The network is very detailed and may be hard to read at a first glance. To
facilitate the reading of the content, we propose to organise the reaction
network as a modular network. The species are clustered in groups, referred to
as modules, in an semi-automatic manner. These modules are then linked by
activating or inhibiting influences according
to the biological knowledge about the interactions between these modules. A
detailed method on the construction of this modular network using BiNoM is
described in the supplementary methods.

Briefly, we first decomposed the global network into its different components,
by using the name semantics of each protein to isolate the sub-networks in which
each protein is involved (function ``get material components''). The 36 networks
that are created this way may share a lot of common species, so we went further
by clustering the subnetworks having at least 25\% overlap. We renamed the 7
clusters obtained with a name that illustrate the content and main function of
the clusters (such as E2F1\_RB, Wee1, etc.). Then, we check the content of each 
module, making modifications if necessary. For example, the module E2F1\_RB is
...
edges connecting them (Figure 8).
...
et al. \cite{novak1998model}.
  ...
  \subsection*{Figure 2 - Apoptosis pathway hierarchical structure}
      BioPAX data extracted from the Reactome database \cite{joshi2005reactome}.
...
while grey nodes indicate reactions.
   ...
   \subsection*{Figure 3 - Subnetworks obtained after pruning the graph}
      (a) molecules associated to the input parts of the network (b) molecules
associated with the output parts.

   \subsection*{Figure 4 - Subnetworks obtained after a decomposition in
material components}
...
different BiNoM functions. Each node (pictured as a greeb octagon), represent a
