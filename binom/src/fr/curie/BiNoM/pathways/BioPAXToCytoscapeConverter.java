...
public class BioPAXToCytoscapeConverter extends BioPAXToSBMLConverter {
    ...
    /**
     * <p>Little auxillary container for BioPAX and GraphDocument object
     *
     */
	...
	*/
    ...
    /*public BipartiteGraph getBipartiteGraph(SbmlDocument sbml) throws Exception{

	BipartiteGraph bgr = new BipartiteGraph();
	...
	for (int i=0;i<sbml.getSbml().getModel().getListOfReactions().getReactionArray().length;i++) {
	    ...
	    //String rtype = r.getReactionAnnotation().getCelldesigner_reactionType().substring(0,3);
	    //String rtype = "";
	    //control cntrl = (control)controls.get(r.getId());
	    //String rtype =
		...
		for (int j=0;j<r.getListOfReactants().getSpeciesReferenceArray().length;j++) {
			...
			//fw.write(nam+"\t"+"LEFT"+"\t"+r.getId()+"\r\n");
			Node n1 = bgr.getCreateNode(id);
			n1.NodeLabel = nam;
			Node n2 = bgr.getCreateNode(r.getId());
			n2.NodeInfo = "";
			n2.NodeClass = 1;
			n2.NodeClassInfo = "REACTION";
			Edge e = new Edge();
			bgr.Edges.add(e);
			e.Node1 = n1;
			e.Node2 = n2;
		...
		for (int j=0;j<r.getListOfProducts().getSpeciesReferenceArray().length;j++) {
			...
			nam = ((SpeciesDocument.Species)species.get(id)).getName().getStringValue();
		    if ((nam!=null)&&(!nam.startsWith("null"))) {
			Node n1 = bgr.getCreateNode(r.getId());
			n1.NodeInfo = "";//r.getReactionAnnotation().getCelldesigner_reactionType();
			Node n2 = bgr.getCreateNode(id);
			n1.NodeClass = 1;
			n1.NodeClassInfo = "REACTION";
			n2.NodeLabel = nam;
			Edge e = new Edge();
			bgr.Edges.add(e);
			e.Node1 = n1;
		...
		for (int j=0;j<r.getListOfModifiers().getModifierSpeciesReferenceArray().length;j++) {
			...
			nam = ((SpeciesDocument.Species)species.get(id)).getName().getStringValue();
		    if ((nam!=null)&&(!nam.startsWith("null"))) {
			//fw.write(nam+"\t"+"MODIFIES"+"\t"+r.getId()+"\r\n");
			Node n1 = bgr.getCreateNode(id);
			Node n2 = bgr.getCreateNode(r.getId());
			n2.NodeInfo = "";//r.getReactionAnnotation().getCelldesigner_reactionType();
			n2.NodeClass = 1;
			n2.NodeClassInfo = "REACTION";
			n1.NodeLabel = nam;
			Edge e = new Edge();
			...
			e.Node1 = n1;
			e.Node2 = n2;
			e.EdgeLabel = "MODIFIER";
			String cid = r.getId()+"_"+id;
			control cntrl = (control)controls.get(cid);
			if (cntrl!=null) {
			    if (cntrl.getCONTROL_DASH_TYPE()!=null);
			    e.EdgeLabel = cntrl.getCONTROL_DASH_TYPE();
			}
		    }
		}
	}
	...
	for (int i=0;i<sbml.getSbml().getModel().getListOfSpecies().getSpeciesArray().length;i++) {
		...
		if (part!=null) {
            ...
            //Utils.addAttribute(n1,"BIOPAX_NODE_ID","BIOPAX_NODE_ID",id,ObjectType.STRING);
		    for(int k=0;k<part.sinonymSpecies.size();k++){
		    	//Utils.addAttribute(n1,"BIOPAX_NODE_ID","BIOPAX_NODE_ID",Utils.cutUri(((physicalEntityParticipant)part.sinonymSpecies.get(k)).uri()),ObjectType.STRING);
		    	Utils.addAttribute(n1,"BIOPAX_URI","BIOPAX_URI",((physicalEntityParticipant)part.sinonymSpecies.get(k)).uri(),ObjectType.STRING);
		    }
		}
		NodeIDs.put(id,n1);
	    }
	...
	pl = biopax_DASH_level2_DOT_owlFactory.getAllinteraction(biopax.model);
	for (int i=0;i<pl.size();i++) { interactions.put(((entity)pl.get(i)).uri(),pl.get(i));   }
	pl = biopax_DASH_level2_DOT_owlFactory.getAllphysicalInteraction(biopax.model);
	for (int i=0;i<pl.size();i++) { interactions.put(((entity)pl.get(i)).uri(),pl.get(i));   }
	pl = biopax_DASH_level2_DOT_owlFactory.getAllcontrol(biopax.model);
	for (int i=0;i<pl.size();i++) { interactions.put(((entity)pl.get(i)).uri(),pl.get(i));   }
	pl = biopax_DASH_level2_DOT_owlFactory.getAllconversion(biopax.model);
	for (int i=0;i<pl.size();i++) { interactions.put(((entity)pl.get(i)).uri(),pl.get(i));   }
	pl = biopax_DASH_level2_DOT_owlFactory.getAllcatalysis(biopax.model);
	for (int i=0;i<pl.size();i++) { interactions.put(((entity)pl.get(i)).uri(),pl.get(i));   }
	pl = biopax_DASH_level2_DOT_owlFactory.getAllmodulation(biopax.model);
	for (int i=0;i<pl.size();i++) { interactions.put(((entity)pl.get(i)).uri(),pl.get(i));   }
	pl = biopax_DASH_level2_DOT_owlFactory.getAllbiochemicalReaction(biopax.model);
	for (int i=0;i<pl.size();i++) { interactions.put(((entity)pl.get(i)).uri(),pl.get(i));   }
	pl = biopax_DASH_level2_DOT_owlFactory.getAlltransportWithBiochemicalReaction(biopax.model);
	for (int i=0;i<pl.size();i++) { interactions.put(((entity)pl.get(i)).uri(),pl.get(i));   }
	pl = biopax_DASH_level2_DOT_owlFactory.getAllcomplexAssembly(biopax.model);
	for (int i=0;i<pl.size();i++) { interactions.put(((entity)pl.get(i)).uri(),pl.get(i));   }
	pl = biopax_DASH_level2_DOT_owlFactory.getAlltransport(biopax.model);
	for (int i=0;i<pl.size();i++) { interactions.put(((entity)pl.get(i)).uri(),pl.get(i));   }
	...
	if (option.includePathways) {
	    ...
	    for (int i=0;i<pl.size();i++) {
		pathwayStep p = (pathwayStep)pl.get(i);
		GraphicNode n1 = addPathwayStepNode(grf,p,pathways,interactions,pathwaySteps,nodes,edges, option);
	    }
	...
	if (n == null) {
	    ...
	    //Utils.addAttribute(n,"BIOPAX_NODE_ID","BIOPAX_NODE_ID",p.uri(),ObjectType.STRING);
	...
	if (n == null) {
	    ...
	    //Utils.addAttribute(n,"BIOPAX_NODE_ID","BIOPAX_NODE_ID",ps.uri(),ObjectType.STRING);
	...
	if (n == null) {
	    ...
	    //Utils.addAttribute(n,"BIOPAX_NODE_ID","BIOPAX_NODE_ID",in.uri(),ObjectType.STRING);
     ...
     * parent method )
     */
    public GraphDocument getXGMMLProteinGraph(String name, BioPAX biopax,
					      Option option) throws Exception {
	GraphDocument gr = GraphDocument.Factory.newInstance();
	GraphicGraph grf = gr.addNewGraph();
	...
	List pl = biopax_DASH_level2_DOT_owlFactory.getAllprotein(biopax.model);
	for (int i=0;i<pl.size();i++) {
		...
		if (!nodes.containsKey(pnm)) {
		    ...
		    /*System.out.println("WARNING!!! DOUBLED NAME: "+pnm+"->"+Utils.cutUri(p.uri()));
		      pnm = pnm+"("+Utils.cutUri(p.uri())+")";
		      GraphicNode n = grf.addNewNode();
		      n.setId(pnm); n.setName(pnm); n.setLabel(pnm);
		      Utils.addAttribute(n,"BIOPAX_NODE_TYPE","BIOPAX_NODE_TYPE","protein",ObjectType.STRING);
		      Utils.addAttribute(n,"BIOPAX_NODE_ID","BIOPAX_NODE_ID",p.uri(),ObjectType.STRING);
	...
	pl = biopax_DASH_level2_DOT_owlFactory.getAllsmallMolecule(biopax.model);
	for (int i=0;i<pl.size();i++) {
	...
	pl = biopax_DASH_level2_DOT_owlFactory.getAllcomplex(biopax.model);
	for (int i=0;i<pl.size();i++) {
		...
		//Utils.addAttribute(n,"BIOPAX_NODE_ID","BIOPAX_NODE_ID",cmplx.uri(),ObjectType.STRING);
	...
	pl = biopax_DASH_level2_DOT_owlFactory.getAllphysicalEntity(biopax.model);
	for (int i=0;i<pl.size();i++) {
	...
	while(inn.hasNext()) {
		...
		//Iterator itc = (BioPAXToSBMLConverter.getPropertyURIs(cm,"COMPONENTS")).iterator();
		Iterator itc = cm.getCOMPONENTS();
		while(itc.hasNext()) {
		    //String ss = (String)itc.next();
		    ...
		    if (n1==null)
			System.out.println("COMPLEX COMPONENT IS NOT INCLUDED: "+Utils.cutUri(pep.getPHYSICAL_DASH_ENTITY().uri()));
		    else {
	...
	List il = biopax_DASH_level2_DOT_owlFactory.getAllconversion(biopax.model);
	for (int i=0;i<il.size();i++) knownInteractions.put(((interaction)il.get(i)).uri(),(interaction)il.get(i));
	il = biopax_DASH_level2_DOT_owlFactory.getAllbiochemicalReaction(biopax.model);
	for (int i=0;i<il.size();i++) knownInteractions.put(((interaction)il.get(i)).uri(),(interaction)il.get(i));
	il = biopax_DASH_level2_DOT_owlFactory.getAlltransport(biopax.model);
	for (int i=0;i<il.size();i++) knownInteractions.put(((interaction)il.get(i)).uri(),(interaction)il.get(i));
	il = biopax_DASH_level2_DOT_owlFactory.getAlltransportWithBiochemicalReaction(biopax.model);
	for (int i=0;i<il.size();i++) knownInteractions.put(((interaction)il.get(i)).uri(),(interaction)il.get(i));
	il = biopax_DASH_level2_DOT_owlFactory.getAllcomplexAssembly(biopax.model);
	for (int i=0;i<il.size();i++) knownInteractions.put(((interaction)il.get(i)).uri(),(interaction)il.get(i));
	il = biopax_DASH_level2_DOT_owlFactory.getAllcontrol(biopax.model);
	for (int i=0;i<il.size();i++) knownInteractions.put(((interaction)il.get(i)).uri(),(interaction)il.get(i));
	il = biopax_DASH_level2_DOT_owlFactory.getAllcatalysis(biopax.model);
	for (int i=0;i<il.size();i++) knownInteractions.put(((interaction)il.get(i)).uri(),(interaction)il.get(i));
	il = biopax_DASH_level2_DOT_owlFactory.getAllmodulation(biopax.model);
	for (int i=0;i<il.size();i++) knownInteractions.put(((interaction)il.get(i)).uri(),(interaction)il.get(i));
	...
	il = biopax_DASH_level2_DOT_owlFactory.getAllphysicalInteraction(biopax.model);
	for (int j=0;j<il.size();j++) {
	    physicalInteraction pi = (physicalInteraction)il.get(j);
		...
		if (v.size()>0) {
		    ...
		    //Utils.addAttribute(n,"BIOPAX_NODE_ID","BIOPAX_NODE_ID",pi.uri(),ObjectType.STRING);
