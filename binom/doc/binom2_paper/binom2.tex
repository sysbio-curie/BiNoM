...
network, named the PIQuant score (Pathway Influence Quantification). The goal of the 
PIQuant score is to
help the user to understand experimental effects associated to the input nodes on the
output nodes, for a given network architecture and some given data. 
Note that the target
node can be a gene or a phenotype of interest, representing a more complex
biological function, such as cell proliferation or apopotosis. For a given network 
...
situations; its adavantage is that there is no need for more detailed knowledge 
than the influence network. The
...
the cell cycle. We selected 19 source nodes as key, well-defined elements of the
cell cycle G1/S transition, and three target nodes as markers of the G1, S and M
phases of the cell cycle. 

In order to annotate the source nodes of this network, we used expression data
...
influenced by a 
mRNA level. When a complex is annotated, it means that at least one of the partners has
