...
/*
 * Small script for removing proteins from CellDesigner files
 */
public class RemoveNonExpressedProteins{
  ...
  public static void main(String[] args) {
    ...
    //String path = "c:/datas/bladdercancer/public/";
    String path = "C:/Datas/CALAMAR/SpecificMap/";
    
    //args[0] = path+"test.xml";
    //args[0] = path+"CC_DNArepair_22_02_2010_corrected_1.xml";
    args[0] = path+"rbmap_mod.xml";
    //args[1] = path+"bc3.dat";
    args[1] = path+"bc3_nonexpressed_av.txt";
    
    /*VDataTable vt = vdaoengine.data.io.VDatReadWrite.LoadFromVDatFile(args[1]);
    ...
    Vector<String> nonExpr = new Vector<String>();
    vdaoengine.data.io.VDatReadWrite.saveToVDatFile(vt, path+"bc3log.dat");
    VDataSet vd = VSimpleProcedures.SimplyPreparedDatasetWithoutNormalization(vt, 57);
    vt.addNewColumn("invasive", "", "", vt.STRING, "0");
    vt.addNewColumn("noninvasive", "", "", vt.STRING, "0");
    vt.addNewColumn("normal", "", "", vt.STRING, "0");
    vt.addNewColumn("allcancers", "", "", vt.STRING, "0");
    vt.addNewColumn("invasive_normal", "", "", vt.STRING, "0");
    vt.addNewColumn("noninvasive_normal", "", "", vt.STRING, "0");
    vt.addNewColumn("allcancers_normal", "", "", vt.STRING, "0");
    for(int i=0;i<vd.pointCount;i++){
    	float x[] = vd.getVector(i);
    	int nonExpressed = 0;
    	for(int j=0;j<x.length;j++)
    		if(x[j]<4f) nonExpressed++;
		String gn = vt.stringTable[i][vt.fieldNumByName("GeneSymbol")];
		if(gn.equals("RB1"))
			System.out.println("RB1 nonexpressed = "+nonExpressed);
    	if(nonExpressed>(int)(57f*0.8f)){
    		//System.out.println(vt.stringTable[i][vt.fieldNumByName("GeneSymbol")]);
    		//if(nonExpr.indexOf(gn)<0)
    		//	nonExpr.add(gn);
    	}
    int invasiveNumber = 0;
    float invasAv = 0f;
    int noninvasiveNumber = 0;
    float noninvasAv = 0f;
    int normalNumber = 0;
    float normalAv = 0f;
    int allCancersNumber = 0;
    float allCancersAv = 0f;
    for(int j=0;j<vt.fieldNames.length;j++)if(vt.fieldTypes[j]==vt.NUMERICAL){
    	float f = Float.parseFloat(vt.stringTable[i][j]);
    	String fn = vt.fieldInfo[j][5];
    	//System.out.println(fn);
    	if(fn.equals("normal")){
    		normalNumber++; normalAv+=f;
    	}
    	if(fn.equals("invasive")){
    		invasiveNumber++; invasAv+=f;
    		allCancersNumber++; allCancersAv+=f;    		
    	}    	
    	if(fn.equals("noninvasive")){
    		noninvasiveNumber++; noninvasAv+=f;
    		allCancersNumber++; allCancersAv+=f;    		
    	}    	
    }
	normalAv/=normalNumber;
	invasAv/=invasiveNumber;
	noninvasAv/=noninvasiveNumber;
	allCancersAv/=allCancersNumber;
	vt.stringTable[i][vt.fieldNumByName("invasive")] = ""+invasAv;
	vt.stringTable[i][vt.fieldNumByName("noninvasive")] = ""+noninvasAv;
	vt.stringTable[i][vt.fieldNumByName("normal")] = ""+normalAv;
	vt.stringTable[i][vt.fieldNumByName("allcancers")] = ""+allCancersAv;
	vt.stringTable[i][vt.fieldNumByName("invasive_normal")] = ""+invasAv/normalAv;
	vt.stringTable[i][vt.fieldNumByName("noninvasive_normal")] = ""+noninvasAv/normalAv;
	vt.stringTable[i][vt.fieldNumByName("allcancers_normal")] = ""+allCancersAv/normalAv;
	if(allCancersAv<4f)
		nonExpr.add(gn);    
    }

    vdaoengine.data.io.VDatReadWrite.saveToVDatFile(vt, path+"bc3log_addcol.dat");
    
    Collections.sort(nonExpr);
    for(int i=0;i<nonExpr.size();i++)
    	System.out.println(nonExpr.get(i));
    System.exit(0);*/


    RemoveNonExpressedProteins cl = new RemoveNonExpressedProteins();
