...
public class ModuleUtils {
	...
	public void checkEdgeColumn(CyNetwork net,String columnTitle,Class<?> type){
		if(net.getDefaultEdgeTable().getColumn(columnTitle)==null) 
			net.getDefaultEdgeTable().createColumn(columnTitle,type,false);
	}
	public void checkNodeColumn(CyNetwork net,String columnTitle,Class<?> type){
		if(net.getDefaultNodeTable().getColumn(columnTitle)==null) 
			net.getDefaultNodeTable().createColumn(columnTitle,type,false);
	}
	...
	static CyNetwork cloneCurrent(String postfix){
		...
		CyNetwork oldNW=Launcher.getAdapter().getCyApplicationManager().getCurrentNetwork();
		CyNetwork newNW=Launcher.getAdapter().getCyNetworkFactory().createNetwork();
		
		
		CyNetwork newNW=Cytoscape.createNetwork(oldNW.nodesList(),oldNW.edgesList(),oldNW.getTitle()+postfix);
		
		CyNetworkView newView=Launcher.getAdapter().getCyNetworkViewFactory().createNetworkView(newNW);
