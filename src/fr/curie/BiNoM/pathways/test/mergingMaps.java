...
public class mergingMaps {
	...
	public static void countAll(SbmlDocument cd){
		...
		Set keys = rt.keySet(); Iterator<String> it = keys.iterator(); 
		Vector<String> types = new Vector<String>(); while(it.hasNext()) types.add(it.next()); Collections.sort(types);
		...
		keys = rt.keySet(); it = keys.iterator(); 
		types = new Vector<String>(); while(it.hasNext()) types.add(it.next()); Collections.sort(types);
		for(int i=0;i<types.size();i++)
			System.out.println(types.get(i)+"\t"+rt.get(types.get(i)));
