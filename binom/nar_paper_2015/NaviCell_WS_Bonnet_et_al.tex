...
Quick live example which \emph{automatically} uploads a sample dataset from TCGA database and illustrates all basic data visualization capabilities of NaviCell Web Service is provided from \href{http://navicell.curie.fr/pages/nav\_web\_service.html} . Sample Python code for quick start on using NaviCell Web Service API is available from the same page in ``Python API and data files" section.
...
are to be prepared using CellDesigner tool\cite{XXX}, which is widely used in systems biology
community and implements a subset of Systems Biology Graphical Notation standard \cite{XXX}
for representing biological networks. These pathway maps are converted into web-based
interactive Google Maps-based interfaces via NaviCell tool\cite{kuperstein2013navicell}.
...
(2) using a large map of Atlas of Cancer Signalling Network (\href{http://acsn.curie.fr}) \cite{XXX} for visualizing ovary cancer data downloaded from The Cancer Genome Atlas \cite{TCGA2011Ovarian} (shown in Supplementary Text ???);
