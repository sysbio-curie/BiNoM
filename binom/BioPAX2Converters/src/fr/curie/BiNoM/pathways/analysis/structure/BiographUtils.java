...
public static Graph CollapseMetaNodes(Graph global, boolean showIntersections, boolean nodeIntersectionView){
  ...
  public static HashMap getNeighborhoodSets(Graph network, Vector<String> selected, boolean goUpstream, boolean goDownstream, int searchRadius, int minimumNumberOfGenes){
	  ...
	  for(int i=0;i<selected.size();i++){
		  ...
		  Vector<Node> nodes = getNodeNeighbors(network,n,goUpstream,goDownstream,searchRadius);
		  Vector<String> names = extractProteinNamesFromNodeNames(nodes);
		  int sr = searchRadius;
		  while(names.size()<minimumNumberOfGenes){
			  sr++;
			  int numberOfNodes = nodes.size();
			  nodes = getNodeNeighbors(network,n,goUpstream,goDownstream,sr);
			  names = extractProteinNamesFromNodeNames(nodes);
