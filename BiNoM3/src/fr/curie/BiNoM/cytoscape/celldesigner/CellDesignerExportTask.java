...
public class CellDesignerExportTask implements Task {
	...
	try {
		...
		if(fileExists&&options.mergeIfFileExists) {
		      ...
		      SbmlDocument sbmlin = CellDesigner.loadCellDesigner(file.getAbsolutePath());
                      ...
                      if(err!=null){
                        JOptionPane.showMessageDialog(null, /*Cytoscape.getDesktop(),*/err+"\n Merge has not been done.");
                        break;
                      }else{
	...
	for (Iterator i = view.getNodeViews().iterator(); i.hasNext(); ) {
	    ...
	    o = Launcher.getAdapter().getCyApplicationManager().getCurrentNetwork().getRow(node).get("CELLDESIGNER_SPECIES", String.class);
	    //o = nodeAttrs.getStringAttribute(node.getIdentifier(),"CELLDESIGNER_SPECIES");
	    if (o != null)
	    	species.add(o);
	    
	    o = Launcher.getAdapter().getCyApplicationManager().getCurrentNetwork().getRow(node).get("CELLDESIGNER_ALIAS", String.class);
	    //o = nodeAttrs.getStringAttribute(node.getIdentifier(),"CELLDESIGNER_ALIAS");
	    if (o != null)
	    	speciesAliases.add(o);
	    
	    o = Launcher.getAdapter().getCyApplicationManager().getCurrentNetwork().getRow(node).get("CELLDESIGNER_REACTION", String.class);
	    //o = nodeAttrs.getStringAttribute(node.getIdentifier(),"CELLDESIGNER_REACTION");
	    if (o != null)
	    	reactions.add(o);
    ...
    public boolean isColorChanged(CyNetwork network){
    	...
    	java.util.Iterator i = network.getNodeList().iterator();
    	...
    	while (i.hasNext()) {
    	    ...
    	    String nodeType = network.getRow(node).get("CELLDESIGNER_NODE_TYPE", String.class);
            try{
                java.awt.Color colorObject = Utils.convertPaintToColor(nv.getVisualProperty(BasicVisualLexicon.NODE_PAINT));
    ...
    public void assignCytoscapeColorsToSpecies(SbmlDocument sbml, CyNetwork network){
    	...
    	while (i.hasNext()) {
            ...
            //System.out.println("NODE COLOR "+id+" : "+rcs+","+gcs+","+bcs);
 	    	...
 	    	String species =  network.getRow(node).get("CELLDESIGNER_SPECIES", String.class);
	...
	public void applyCellDesignerOptions(SbmlDocument sbml, CellDesignerExportToFileDialog.CellDesignerExportToFileOptions options){
	    ...
	    if(options.changeSpeciesCoordinates){
	       	GraphDocument gr = GraphDocumentFactory.getInstance().createGraphDocument(Launcher.getAdapter().getCyApplicationManager().getCurrentNetwork());
	  ...
	  public static void ScaleSizes(SbmlDocument sbml, float factor, boolean onlyDistances){
        ...
        public void assignCytoscapeColorsToReactions(SbmlDocument sbml, CyNetwork network){
            ...
            while (i.hasNext()) {
                ...
                String reaction =  cyNetwork.getRow(node).get("CELLDESIGNER_REACTION", String.class);
            ...
            i = network.getEdgeList().iterator();
            ...
            while (i.hasNext()) {
                ...
                try{
                java.awt.Color colorObject = Utils.convertPaintToColor(ev.getVisualProperty(BasicVisualLexicon.EDGE_PAINT));
                ...
                String mod_species =  cyNetwork.getRow(edge).get("CELLDESIGNER_SPECIES", String.class);
                //String source_target = mod_species+"%%"+target.getIdentifier();
                String source_target = cyNetwork.getRow(target).get(CyNetwork.NAME, String.class);
