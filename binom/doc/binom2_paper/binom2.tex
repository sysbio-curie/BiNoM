...
processes \cite{saraiya2005visualizing, gehlenborg2010visualization}. The
development of those pathway repositories has also fueled the creation of
standard representations and formats, to facilitate the exchange and
representation of data, such as the Biological Pathway Exchange standard
(BioPAX) \cite{demir2010biopax}, the Systems Biology Markup Language (SBML)
\cite{hucka2003systems} or the Systems Biology Graphical Notation (SBGN)
\cite{le2009systems}. The Pathway resource list website counts more than 300
web-accessible biological pathway and network databases
...
source nodes, to a set of 
target nodes) is defined as:
...
It is possible that a path has intermediate nodes that are annotated nodes, 
...
paths for calculating the PIQuant score. In the example above, if only consistent paths 
are kept, PIQuant score of AC to Ph becomes:

$$
 PIQuant_{Score} = 2 \cdot 1 \cdot \frac{1}{3} = 0.67
$$
The biological interpretation of inconsistent paths removal can be detailed, using this example. 
According to the network, there is an influence of Ac to Ph that goes through D. But if this path
is effective, D annotation would have the same sign than Ac annotation, because Ac activates D indirectly. 
In our case, the sign of D annotation is opposite to the sign of AC annotation, meaning that there is another 
(unknown) effect on D that blocks the signal. Therefore, the path from Ac to Ph that goes through D should not 
be used to compute PIQuant score. Practically, we think that this option should be often used, keeping
its biological interpretation in mind.
...
is assessed by a t-test. We construct a text file listing nodes of the influence network 
and their annotation. We assess annotations only to nodes that can be directly influenced by a 
mRNA level. When a complex is annotated, it means that one of the partner has no unbound state in the network. 
We import this file using Cytoscape function 
...
PIQuant is applied to this network and its annotation: we 
run ``Plugins'' $>$ ``BiNoM 2.0'' $>$ ``BiNoM Analysis'' $>$ ``Path Influence Quantifiacation analysis''. 
We take all annotated nodes as active nodes, 
took the grey nodes of Figure~\ref{InflAnnotNet} as targets, selected optimal and sub-optimal 
shortest path. We run a test significance and get the report table (``Get report''). We extract 
the ``Node influence table'' that represent the PIQuant score for each annotated nodes to each 
cell-cycle phase markers. This is represented in Figure~\ref{PIQuantHeatMap}. The global effect illustrated
by this figure is expected: the effect of the annotation to the cell-cycle phases is mainly positive,
implying that proliferation is enhanced in tumor cell.   
