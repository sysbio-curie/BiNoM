...
public class SubnetworkProperties {
	...
	public double networkDistanceMatrix[][] = null;
	...
	public static double[][] connectByShortestPaths(Graph subgraph, Graph network){
		double distMat[][] = new  double[subgraph.Nodes.size()][subgraph.Nodes.size()];
		Vector<Node> vn = new Vector<Node>(); 
		for(int i=0;i<subgraph.Nodes.size();i++){
			vn.add(subgraph.Nodes.get(i));
		}
		for(int i=0;i<vn.size();i++)
		for(int k=0;k<vn.size();k++)if(i!=k){
			distMat[i][k] = 10000; 	
	    }		
		for(int i=0;i<vn.size()-1;i++){
			...
			System.out.print((i+1)+"/"+(vn.size()-1)+"\t");
			//Vector<Graph> v = GraphAlgorithms.DijkstraAlgorithm(network, n, vt, false, Float.MAX_VALUE);
			Vector<Vector<Vector<Integer>>> v = GraphAlgorithms.DijkstraAlgorithmReturnIndexReverseOrder(network, n, vt, false, Float.MAX_VALUE);
			
			double shortestDist = Double.MAX_VALUE;
			int shortestPath = -1;
			if(v.size()>0){
				for(int k=0;k<v.size();k++){
					Vector<Vector<Integer>> vk = v.get(k);
					double d = 10000; 
					if(vk.size()>0){
						d = vk.get(0).size()-1;
					//double d = v.get(k).Nodes.size()-1;
					//int j = vn.indexOf(v.get(k).Nodes.get(v.get(k).Nodes.size()-1));
					int j = vk.get(0).get(0);
					j = vn.indexOf(network.Nodes.get(j));
					distMat[i][j] = d;		distMat[j][i] = d;
					if(d<shortestDist){
						shortestDist = d;
						shortestPath = k;
					}}
			...
			for(int k=0;k<v.size();k++){
				double d = 10000; 
				if(v.get(k).size()>0)
					d = v.get(k).get(0).size()-1;
				if(Math.abs(d-shortestDist)<1e-10){
						//subgraph.addNodes(v.get(k));
					    for(int s=0;s<v.get(k).size();s++)
					    	for(int t=0;t<v.get(k).get(s).size();t++)
					    		subgraph.addNode(network.Nodes.get(v.get(k).get(s).get(t)));
						//System.out.println(v.get(k).Nodes.get(0).Id+" connected to "+v.get(k).Nodes.get(v.get(k).Nodes.size()-1).Id+" dist="+d);
					    System.out.println(network.Nodes.get(v.get(k).get(0).get(v.get(k).get(0).size()-1)).Id+" connected to "+network.Nodes.get(v.get(k).get(0).get(0)).Id+" dist="+d);
					}
				}
		}
		subgraph.addConnections(network);
	...
	public static double[] calcNodeBetweenness(Graph graph, Vector<String> selected, boolean directed, boolean assignAttribute){
		if(selected.size()==0){
			for(int i=0;i<graph.Nodes.size();i++)
				selected.add(graph.Nodes.get(i).Id);
		}
		double centralities[] = new double[graph.Nodes.size()];		
		//for(int i=0;i<graph.Nodes.size()-1;i++){
		for(int i=0;i<selected.size()-1;i++){
			//Node n = graph.Nodes.get(i);
			Node n = graph.getNode(selected.get(i));
			System.out.println(""+i+"/"+graph.Nodes.size()+": Calc centrality for "+n.Id);			
			Vector<Node> vt = new Vector<Node>();
			//for(int j=i+1;j<graph.Nodes.size();j++){
			//	vt.add(graph.Nodes.get(j));
			//}
			for(int j=i+1;j<selected.size();j++){
				vt.add(graph.getNode(selected.get(j)));
			}
			Vector<Vector<Vector<Integer>>> v = GraphAlgorithms.DijkstraAlgorithmReturnIndexReverseOrder(graph, n, vt, directed, Float.MAX_VALUE);
			int totalNumberOfPath = 0;
			if(v.size()>0){
				for(int k=0;k<v.size();k++)for(int k1=0;k1<v.get(k).size();k1++)
					totalNumberOfPath++;
				System.out.println(""+totalNumberOfPath+" paths found");
				for(int k=0;k<v.size();k++)for(int k1=0;k1<v.get(k).size();k1++){
					Vector<Integer> path = v.get(k).get(k1);
					if(path.size()>2)
						for(int j=1;j<path.size()-1;j++){
							int inter_ind = path.get(j);
							//String id = graph.Nodes.get(inter_ind).Id;
							//inter_ind = selected.indexOf(id);
							//if(inter_ind!=-1)
								centralities[inter_ind]+=1.0/(double)v.get(k).size();
						}
				}
			}
		}
		//int normFactor = selected.size();
		int normFactor = graph.Nodes.size();
		for(int i=0;i<centralities.length;i++)
			if(directed)
				centralities[i]/=(normFactor-1)*(normFactor-2);
			else
				centralities[i]/=(normFactor-1)*(normFactor-2)/2;
		if(assignAttribute){
			for(int i=0;i<selected.size();i++){
				Node n = graph.getNode(selected.get(i));
				n.setAttributeValueUnique("INBETWEENNESS", ""+centralities[i],Attribute.ATTRIBUTE_TYPE_REAL);
			}
		}
		return centralities;
	}
	...
	public void connectByShortestPaths(){
		this.subnetworkDistanceMatrix = connectByShortestPaths(subnetwork, network);
	}
	...
	public static void calcSignificanceVsNumberOfGenes(Graph network, Vector<String> rankedListOfProteins, int numberOfPermutations, int nga[]) throws Exception{
