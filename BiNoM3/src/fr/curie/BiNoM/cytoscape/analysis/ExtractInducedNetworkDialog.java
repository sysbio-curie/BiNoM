...
//		    	try{
		    		...
		    		CyNetwork newNetwork = Launcher.getAdapter().getCyNetworkFactory().createNetwork();
		    		...
		    		while(iterNodes.hasNext()){
		    			...
		    			NetworkUtils.addNodeAndReportPosition(node, current, currentView, newNetwork, newNetworkView);
		    		}
		    		...
		    		for(CyEdge edge:edgeList){
		    			if(nodesNamesToAdd.contains(current.getRow(edge.getSource()).get("name", String.class)))
		    				if(nodesNamesToAdd.contains(current.getRow(edge.getTarget()).get("name", String.class)))
								try {
									NetworkUtils.addEd(newNetwork, current, edge);
								} catch (Exception e1) {
