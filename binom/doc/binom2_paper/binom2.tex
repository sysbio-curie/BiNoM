...
%
% AZ: check description of shortest path and sub-optimal path search and add 
% a description for all non intersecting paths
% 
...
\subsection*{Application of PIQuant on RB influence network, data from Stransky.}
%
% PIQuant analysis here (GS)
%
As described above, PIQuant algorithm confront data to an influnce network. Here, we apply PIQuant algorithm to an influence version of the network above, exctracted by BIOCHAM(ref). The data are extracted from transcriptome data of bladder cancer tissu (compared to normal tissu ref). More precisely, for each gene, the t-statistics is computed (between cancer and normal tissu). These data are considered as nodes annotation and are implemented in the cytoscape session of the influence network. Figure~\ref{InflAnnotNet} represent this influence network with the annotation.  

\begin{figure}[h]
  \caption{\label{InflAnnotNet} \textbf{Annoted influence network.}
    Influence network opbtained from BIOCHAM. Color represent differential expression obtained from transcrpotme data (red for positive, green for negative). The three grey nodes act on the three cell cycle checkpoints, G1, S, M.}
