...
public class Transpath {
  ...
  public void populateModel() throws Exception{
    ...
    for(int i=0;i<ga.length;i++){
      ...
      dna g = biopax_DASH_level2_DOT_owlFactory.createdna(biopax.namespaceString+id+"e",biopax.biopaxmodel);
      g.setNAME(name);
      if(ga[i].getSpecies()!=null)
  ...
  public void associateWithSpecies(physicalEntity ent, String species) throws Exception{
    ...
    while(st.hasMoreTokens()){
      ...
      if(sp.trim().length()>2){
        ...
        if(bs==null){
          bs = biopax_DASH_level2_DOT_owlFactory.createbioSource(biopax.namespaceString+Utils.correctName(sp),biopax.biopaxmodel);
          bs.setNAME(sp);
          entities.put(sp,bs);
        }
