...
public class DoubleNetworkDifferenceDialog extends JDialog implements ActionListener {
	...
	setLocation((screenSize.width - (int)getSize().getWidth()) / 2,
                    (screenSize.height - (int)getSize().getHeight()) / 2);
	setVisible(true);
    }
	...
	okB.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			*/
			...
			netw = op.eval(netw, netView);
			Launcher.getAdapter().getCyNetworkViewManager().addNetworkView(netView);
			
			Collection<CyNetworkView> networkViews = Launcher.getAdapter().getCyNetworkViewManager().getNetworkViews(netw1);
			...
			netw = Launcher.getAdapter().getCyNetworkFactory().createNetwork();
			org.cytoscape.view.model.CyNetworkView netView2 = Launcher.getAdapter().getCyNetworkViewFactory().createNetworkView(netw);
			netw.getRow(netw).set(CyNetwork.NAME, diff2TF.getText());
			...
			*/
			netw = op.eval(netw, netView2);
		
			Launcher.getAdapter().getCyNetworkViewManager().addNetworkView(netView2);
			
			networkViews = Launcher.getAdapter().getCyNetworkViewManager().getNetworkViews(netw2);
			networkViewIterator = networkViews.iterator();
			fromView =null;
			while(networkViewIterator.hasNext())
				fromView = networkViewIterator.next();
			
			vs = Launcher.getAdapter().getVisualMappingManager().getVisualStyle(fromView);			
			Launcher.getAdapter().getVisualMappingManager().setVisualStyle(vs, netView2);
    ...
    public void actionPerformed(ActionEvent e) {
		...
		if (idx1 == idx2) {
			...
			diff1TF.setText(netw1.getRow(netw1).get(CyNetwork.NAME, String.class) + " - " + netw2.getRow(netw2).get(CyNetwork.NAME, String.class));
			diff2TF.setText(netw2.getRow(netw2).get(CyNetwork.NAME, String.class) + " - " + netw1.getRow(netw1).get(CyNetwork.NAME, String.class));
		    }
		}
    }
}
