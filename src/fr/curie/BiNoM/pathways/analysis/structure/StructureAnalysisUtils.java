...
public class StructureAnalysisUtils {
  ...
  public static Vector getMaterialComponents(GraphDocument graph){
    Vector res = new Vector();
    Graph gr = XGMML.convertXGMMLToGraph(graph);
    Vector comps = BiographUtils.calcAllMaterialComponents(gr);
    for(int i=0;i<comps.size();i++){
      Graph gr1 = (Graph)comps.elementAt(i);
      gr1.name = gr.name+"_"+gr1.name;
      GraphDocument xgr = XGMML.convertGraphToXGMML(gr1);
      res.add(xgr);
    }
    return res;
  ...
  public static GraphDocument getModularView(GraphDocument graph, Vector<GraphDocument> modules, boolean showIntersections, boolean nodeIntersectionView){
	  GraphDocument gr = null;
	  Vector graphs = new Vector();
	  for(int i=0;i<modules.size();i++){
		  Graph module = XGMML.convertXGMMLToGraph(modules.get(i));
		  graphs.add(module);
	  }
	  Graph global = XGMML.convertXGMMLToGraph(graph);
	  global.metaNodes = graphs;
	  Graph meta = BiographUtils.CollapseMetaNodes(global, showIntersections, nodeIntersectionView);
	  gr = XGMML.convertGraphToXGMML(meta);
	  return gr;
  }
