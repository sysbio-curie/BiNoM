...
private DedalMethods dMet = new DedalMethods();
	public TransitionalLayout(CySwingAppAdapter adapter){
		// Add a menu item
		super("Transformation between two networks",
...
for (CyNetwork cyNet: newList){
	...
	for (CyNetwork net : allNets){
		...
		//network.getRow(network).set(CyNetwork.NAME, "Transformation Layout");
	
		network.getRow(network).set(CyNetwork.NAME, "Transformation Layout Network");
