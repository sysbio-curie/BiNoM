...
file formats, but The BiNoM plugin is not designed to be a universal converter.
The goal is more
...
influence of a source node on a target node, called PIQuant.
...
\color{red}In this version of BiNoM, we have introduced a novel approach to quantify the
effect of experimental data onto one or more target nodes for a given
network, named the PIQuant score (Pathway Influence Quantification). 
The target
node can be a gene or a phenotype of interest, representing a more complex
biological function, such as cell proliferation or apopotosis. A positive/negative
PIQuant score is a quantitive theoretical prediction of over/underexpression of 
the target node. For instance, consider 
experimental data consisting of differential gene expressions 
(between a perturbed biological state compared to normal) and the output node as
a phenotype. In that case, positive/negative PIQuant score predicts quantitatively
that the phenotype would be enhanced/inhibited. The PIQuant score itself is difficult 
to interpret directly; it is easier to use it in order to compare two sets
of data or two networks, according to the effect on the target node. \color{black}
...
not require extra parameters other than the influence newtwork to be calculated.
...
\textit{inconsistent}. \color{red} Practically, we offer the option to
keep or not the inconsistent paths for the calculation of the PIQuant score,
depending on how the user wants to analyze the results: by removing inconsistent path
the user considers that no information can pass through the inconsistent paths, they
are blocked by the annotation. In other words, it is a way to ``contextualize'' the network
by considering that some of paths are not active, in the context defined by the annotation.
In the contrary, by keeping inconsistent paths, the user wants to have a quantitive prediction on an output
node in a more general context, when all possible connections are potentially active.

(The rationale behind these definitions is that the
network structure for intermediate nodes can be either consistent or inconsistent with
experimental data values ?).

According to the network, there is an influence from Ac to Ph
...
might not be used to compute PIQuant score.
...
every possible path found by the algorithm. We describe a detailed
...
simplified versions of it along with an interactive (clickable) map is
...
the cell cycle. We selected three target nodes as markers of the G1, S and M
phases of the cell cycle. We selected 19 source nodes of this network: they are 
the ones that can be directly influence by a mRNA level. Some of these source 
nodes are complexes: the mRNA level that influence such a complexe is the one of 
the partners that has no unbound state in the network.

For each of these source nodes, we used expression data from a study of 57 bladder cancer
tissue samples compared to 4 normal samples \cite{stransky2006regional}. For each 
...
and their annotation. \color{black}
We imported this file using the Cytoscape function 
...
different cell-cycle phases: G1 (PRB\_star), S (CDK2/Cyclin E1 complex), M (CDC2/Cyclin E1 complex).}
...
score values are available as supplementary table 1). We selected optimal and
sub-optimal shortest path as the algorithm to extract the paths. We ran a test
significance and got the report table. In this report, the ``Node influence
...
construction of this modular network using BiNoM is described in the supplementary methods.
