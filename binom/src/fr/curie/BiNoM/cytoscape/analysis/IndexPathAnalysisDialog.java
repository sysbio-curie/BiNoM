...
public class IndexPathAnalysisDialog extends JDialog {
	...
	while(it.hasNext())
	    data.add(((CyNode)it.next()).getIdentifier());
		...
		public void actionPerformed(ActionEvent e) {
			...
			IndexPathAnalysisTask task = new IndexPathAnalysisTask
			    (sources,targets,options,
			     Cytoscape.getCurrentNetworkView().getVisualStyle(),outputCurrentNetworkRB.isSelected());
			...
			for (Iterator i = view.getNodeViewsIterator(); i.hasNext(); ) {
			    ...
			    nView.setSelected(task.SelectedNodes.contains(node.getIdentifier()));
			}
