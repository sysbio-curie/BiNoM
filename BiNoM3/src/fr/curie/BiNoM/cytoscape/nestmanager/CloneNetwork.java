...
import fr.curie.BiNoM.cytoscape.nestmanager.GroupUtils;
...
public class CloneNetworkTask extends AbstractCreationTask implements ObservableTask {
	...
	public CyNetwork cpy(CyNetwork net){
		
		this.netFactory = Launcher.getAdapter().getCyNetworkFactory();
		this.netViewFactory = Launcher.getAdapter().getCyNetworkViewFactory();
		//this.naming = Launcher.getCySwingAppAdapter().getCyNetworkFactory().;
		this.appMgr = Launcher.getAdapter().getCyApplicationManager();
		//this.netTableMgr = Launcher.getAdapter().getcyni;
		this.rootNetMgr = Launcher.getAdapter().getCyRootNetworkManager();
		this.groupMgr = Launcher.getAdapter().getCyGroupManager();
		this.groupFactory = Launcher.getAdapter().getCyGroupFactory();
		this.renderingEngineMgr = Launcher.getAdapter().getRenderingEngineManager();
