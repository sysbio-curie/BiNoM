...
public class BioPAXClassDescFactory {
	...
	*/
    ...
    /*
    public void makeClassesDesc_old(BioPAX biopax) {
	if (biopax == null)
	    return;
	Iterator<Statement> iter = biopax.model.listStatements();
	while (iter.hasNext()) {
	    Statement st = iter.next();
	    RDFNode node = st.getObject();
	    if (node.isURIResource()) {
		try {
		    com.ibm.adtech.jastor.Thing thing = biopax_DASH_level2_DOT_owlFactory.getThing(node.toString(), biopax.model);
		    int cnt = thing.listStatements().size();
		    if (cnt > 0) {
			getClassDesc(BioPAXPropertyUtils.getClass(thing.getClass()));
		    }
		}
