...
public class UpdateInfluenceAttrib extends AbstractCyAction {
	...
	public UpdateInfluenceAttrib(){
		super(UpdateInfluenceAttrib.title,
        		Launcher.getAdapter().getCyApplicationManager(),
            "pippo",
            Launcher.getAdapter().getCyNetworkViewManager());
        setPreferredMenu("Plugin.BiNoM 3.Fading Signal Propagation Model");
	}
	...
	void fillAttributesValues(){
		 ...
		 while(columnsIterator.hasNext()){
	    	...
	    	if(col.getType().isInstance(String.class))
	    		fields.add(col.getName());
			 	datas.add(new ArrayList<String>());	 
		 }
//		 for(int i=0;i<attrNames.length;i++) 
//			 if(attrib.getType(attrNames[i])==CyAttributes.TYPE_STRING){
...
//		 for(int i=0;i<fields.size();i++){
//			 for(CyEdge edge:NestUtils.getEdgeList(network)){
//				 String attrStr=attrib.getStringAttribute(edge.getSource().getNetworkPointer().getRow(edge).get(CyNetwork.NAME, String.class),fields.get(i));
//				 if(!datas.get(i).contains(attrStr)&&(attrStr!=null)) 
//					 datas.get(i).add(attrStr); 
//			 }
//		 }
	...
	public void actionPerformed(ActionEvent e){
		...
		DRComboBox dialog=new DRComboBox(null, /*Cytoscape.getDesktop()*/title,label,fields,selected,datas);
		if(dialog.launchDialog(data)){
			int unknown=update();
			if(unknown!=0) JOptionPane.showMessageDialog(null, /*Cytoscape.getDesktop()*/unknown+" edges with unknown influence",
