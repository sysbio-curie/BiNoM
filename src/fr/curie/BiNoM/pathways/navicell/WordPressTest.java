...
public class WordPressTest {
	...
	@SuppressWarnings({ "nls", "boxing" })
		...
		try {
			...
			options.addOption("id", "recentpostids", true, "Get recent posts IDs");
					...
					try {
						...
						if (config.hasOption("authors")) {
							...
							printList(wp.getRecentPosts(getInteger("recentposts", config)),
								  Page.class, false);
						} else if (config.hasOption("recentpostids")) {
							printPostids(wp.getRecentPosts(getInteger("recentpostids", config)));
							...
							if (!config.hasOption("publish")) {
								...
								String prop = System.getProperty("ntimes");
								int ntimes = prop != null ? Integer.valueOf(prop) : 1;
								for (int nn = 0; nn < ntimes; ++nn) {
									String status = wp.newPost(Page.fromFile(new File(config.getOptionValue("newpost"))), Boolean.valueOf(config.getOptionValue("publish")));
	...
	private static List<Page> getAllPosts(Wordpress wp) throws XmlRpcFault {
		if (!WORDPRESS_XMLRPC_PATCHED) {
			return wp.getRecentPosts(10000000);
		}

		// EV patched protocol
		List<Page> allPosts = new java.util.LinkedList<Page>();
		final int NUMBER_OF_POSTS = 0x400;
	...
	@SuppressWarnings({ "nls", "boxing" })
		...
		String post_ID = config.getOptionValue(opt);
		String publish = config.getOptionValue("publish");
		if ((publish != null) && (post_ID != null)) {
			if (deletePage) {
				System.out.println(wp.deletePage(Integer.valueOf(post_ID), publish));
			} else {
				if (post_ID.equals("*")) {
					//List<Page> pages = wp.getRecentPosts(100000);
					List<Page> pages = getAllPosts(wp);
					for (Page o : pages) {
						Boolean status = wp.deletePost(o.getPostid(), publish);
						System.out.println("deleting:" + o.getPostid() + " " + status);
					}
					System.out.println(pages.size() + " deleted");
				} else {
					String[] post_IDs = post_ID.split(":");
					for (String pid : post_IDs) {
						Boolean status = wp.deletePost(Integer.valueOf(pid), publish);
						System.out.println("deleting: " + pid + " " + status);
					}
				}
			}
		} else {
	...
	private static void printPostids(List<Page> r) {
		System.out.println("list size: " + r.size());
		boolean headerPrinted = false;
		for (Page o : r) {
			System.out.println(o.getPostid());
		}
		System.out.println("list size: " + r.size());
	}
}
