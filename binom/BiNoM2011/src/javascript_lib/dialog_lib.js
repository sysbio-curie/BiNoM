...
function skip_datatable(datatable)
{
    return datatable.isModuleScores();
    //return false;
}
...
function get_true_datatable_count()
{
    var datatable_cnt = 0;
    for (var dt_name in navicell.dataset.datatables) {
	var datatable = navicell.dataset.datatables[dt_name];
...
function update_sample_status_table(doc, params) {
    ...
    var datatable_cnt = get_true_datatable_count();
    // should use a string buffer
    var str = "<thead>";
    str += "<tr><td>&nbsp</td>" + (datatable_cnt > 0 ? "<td colspan='" + mapSize(navicell.dataset.datatables) + "'>&nbsp;#Genes&nbsp;for&nbsp;sample&nbsp;in&nbsp;datatable&nbsp;</td>" : "") + "</tr>";
...
function update_module_status_table(doc, params) {
    ...
    var datatable_cnt = get_true_datatable_count();
    var str = "<thead>";
    str += "<tr><td>&nbsp</td>" + (datatable_cnt > 0 ? "<td colspan='" + mapSize(navicell.dataset.datatables) + "'>&nbsp;#Genes&nbsp;in&nbsp;</td>" : "") + "</tr>";
    str += "<tr><th>Modules (" + (mapSize(navicell.mapdata.module_mapdata)+mapSize(navicell.mapdata.module_ids)) + ")</th>";
    for (var datatable_name in navicell.dataset.datatables) {
	var datatable = navicell.dataset.datatables[datatable_name];
	if (skip_datatable(datatable)) {
	    continue;
	...
	for (var datatable_name in navicell.dataset.datatables) {
	    ...
	    str += "<td style='text-align: center'>" + gene_cnt + "</td>";
	}
	str += "</tr>";
    }
    for (var id_module_name in navicell.mapdata.module_ids) {
	str += "<tr><td>" + id_module_name + "</td>";
	for (var datatable_name in navicell.dataset.datatables) {
	    var datatable = navicell.dataset.datatables[datatable_name];
	    if (skip_datatable(datatable)) {
		continue;
	    ...
	    var gene_cnt = 0;
	    for (var gene_name in datatable.gene_index) {
		var id_module_list = navicell.mapdata.hugo_modules[gene_name];
		if (id_module_list) {
		    for (var nn = 0; nn < id_module_list.length; nn++) {
			if (id_module_name == id_module_list[nn]) {
			    gene_cnt++;
			    break;
			}
		    }
...
function update_gene_status_table(doc, params) {
    ...
    str += "<tr><td colspan='" + (module_stack.length) + "'>Genes&nbsp;in</td>" + (datatable_cnt > 0 ? "<td colspan='" + mapSize(navicell.dataset.datatables) + "'>&nbsp;#Samples&nbsp;for&nbsp;gene&nbsp;in&nbsp;datatable&nbsp;</td>" : "") + "</tr>";
    str += "<tr>";
...
function update_buttons()
{
    ...
    console.log("update_buttons: " + enable);
...
function update_status_tables(params) {
    console.log("Update_Status_Tables");
...
Datatable.prototype.showDisplayConfig = function(doc, what) {
    ...
    console.log("module " + module + " " + displayContinuousConfig + " " + displayUnorderedDiscreteConfig);
...
function map_staining_step_display_config(what, map_name) {
	...
	//var datatable = navicell.getDatatableById(val);
	var datatable = navicell.getDatatableByCanonName(val);
	console.log("name " + val + " " + datatable);
	if (datatable) {
	    var doc = (map_name && maps ? maps[map_name].document : null);
	    datatable.showDisplayConfig(doc, what);
	}
...
function draw_voronoi(module, context, div, image_mode, delta_x, delta_y)
{
    ...
    var voronoi_shape_map = navicell.mapdata.getVoronoiCells(module).getShapeMap();
    console.log("draw_voronoi " + mapSize(voronoi_shape_map));
    context.lineWidth = 1;
	...
	console.log("modifs_id " + modifs_id);
	var color;
	if (!modifs_id) {
	    color = 0;
	} else {
