...
public class CellDesignerToBioPAXConverter {
  ...
  public BioPAX populateModel(){
    ...
    pathway path = biopax_DASH_level2_DOT_owlFactory.createpathway(biopax.namespaceString+sbml.getSbml().getModel().getId(),biopax.biopaxmodel);
    path.setNAME(sbml.getSbml().getModel().getId());
    ...
    /*for(int i=0;i<sbml.getModel().getListOfSpecies().getSpeciess().size();i++){
      ...
      if(an!=null){
        ...
        if(cl.equals("PROTEIN")){
          protein prot = biopax_DASH_level2_DOT_owlFactory.createprotein(biopax.namespaceString+sp.getId(),biopax.biopaxmodel);
          prot.setNAME(convertSpeciesToName(sbml,sp.getId(),true,true));
          prot.setSHORT_DASH_NAME(sp.getName());
        }
        if(cl.equals("ION")){
          smallMolecule sm = biopax_DASH_level2_DOT_owlFactory.createsmallMolecule(biopax.namespaceString+sp.getId(),biopax.biopaxmodel);
    ...
    for(int i=0;i<species.size();i++){
      ...
      if(sp.getAnnotation()!=null)if(sp.getAnnotation().getCelldesignerSpeciesIdentity()!=null)
        if(Utils.getValue(sp.getAnnotation().getCelldesignerSpeciesIdentity().getCelldesignerClass()).equals("PHENOTYPE")){
          ...
          protein pheno = biopax_DASH_level2_DOT_owlFactory.createprotein(biopax.namespaceString+sp.getId(),biopax.biopaxmodel);
          pheno.setNAME(sp.getName().getStringValue());
          pheno.setSHORT_DASH_NAME(sp.getName().getStringValue());
          String cleanname = correctName(sp.getName().getStringValue());
          pheno.addSYNONYMS(cleanname);
    ...
    for(int i=0;i<species.size();i++){
      ...
      if(sp.getAnnotation()!=null)if(sp.getAnnotation().getCelldesignerSpeciesIdentity()!=null)
        if(Utils.getValue(sp.getAnnotation().getCelldesignerSpeciesIdentity().getCelldesignerClass()).equals("UNKNOWN")){
          ...
          physicalEntity pheno = biopax_DASH_level2_DOT_owlFactory.createphysicalEntity(biopax.namespaceString+sp.getId(),biopax.biopaxmodel);
          pheno.setNAME(sp.getName().getStringValue());
          pheno.setSHORT_DASH_NAME(sp.getName().getStringValue());
          String cleanname = correctName(sp.getName().getStringValue());
          pheno.addSYNONYMS(cleanname);
    ...
    for(int i=0;i<proteins.size();i++){
      ...
      protein protein_ = biopax_DASH_level2_DOT_owlFactory.createprotein(biopax.namespaceString+prot.getId(),biopax.biopaxmodel);
      protein_.setNAME(prot.getName().getStringValue());
      protein_.setSHORT_DASH_NAME(prot.getName().getStringValue());
      String cleanname = correctName(prot.getName().getStringValue());
      protein_.addSYNONYMS(cleanname);
    ...
    for(int i=0;i<sbml.getSbml().getModel().getAnnotation().getCelldesignerListOfGenes().getCelldesignerGeneArray().length;i++){
      ...
      dna dna_ = biopax_DASH_level2_DOT_owlFactory.createdna(biopax.namespaceString+gene.getId(),biopax.biopaxmodel);
      dna_.setNAME(gene.getName());
      dna_.addSYNONYMS(gene.getName());
    ...
    for(int i=0;i<sbml.getSbml().getModel().getAnnotation().getCelldesignerListOfRNAs().getCelldesignerRNAArray().length;i++){
      ...
      rna rna_ = biopax_DASH_level2_DOT_owlFactory.createrna(biopax.namespaceString+RNA.getId(),biopax.biopaxmodel);
      rna_.setNAME(RNA.getName());
      rna_.addSYNONYMS(RNA.getName());
    ...
    for(int i=0;i<sbml.getSbml().getModel().getAnnotation().getCelldesignerListOfAntisenseRNAs().getCelldesignerAntisenseRNAArray().length;i++){
      ...
      rna rna_ = biopax_DASH_level2_DOT_owlFactory.createrna(biopax.namespaceString+aRNA.getId(),biopax.biopaxmodel);
      rna_.setNAME(aRNA.getName());
      rna_.addSYNONYMS(aRNA.getName());
    ...
    for(int i=0;i<sbml.getSbml().getModel().getListOfSpecies().getSpeciesArray().length;i++){
      ...
      if(an!=null){
        ...
        if(cl.equals("ION")){
          ...
          smallMolecule sm = biopax_DASH_level2_DOT_owlFactory.createsmallMolecule(biopax.namespaceString+sp.getId(),biopax.biopaxmodel);
          sm.setNAME(CellDesignerToCytoscapeConverter.convertSpeciesToName(sbml,sp.getId(),false,true));
          sm.setSHORT_DASH_NAME(sp.getName().getStringValue());
          String cleanname = correctName(sp.getName().getStringValue());
          sm.addSYNONYMS(cleanname);
    ...
    for(int i=0;i<sbml.getSbml().getModel().getAnnotation().getCelldesignerListOfComplexSpeciesAliases().getCelldesignerComplexSpeciesAliasArray().length;i++){
      ...
      if(complexes.indexOf(complex_id)<0){
        complex compl = biopax_DASH_level2_DOT_owlFactory.createcomplex(biopax.namespaceString+complex_id,biopax.biopaxmodel);
        compl.setNAME(complex_name);
        //compl.setSHORT_DASH_NAME(complex_name);
        compl.setSHORT_DASH_NAME(CellDesignerToCytoscapeConverter.getCelldesignerSpeciesName(sbml.getSbml(),csa.getSpecies()));
        ...
        for(int j=0;j<sbml.getSbml().getModel().getAnnotation().getCelldesignerListOfIncludedSpecies().getCelldesignerSpeciesArray().length;j++){
          ...
          if(cs.equals(csa_sp_id)){
            physicalEntityParticipant pep = createEntityParticipantForSpecies(isp.getId(),species,complexes_list,physEntities,sbml.getSbml(),biopax,csa_id);
            compl.addCOMPONENTS(pep);
          }
        }
      }
      ...
      if(sp.getAnnotation().getCelldesignerSpeciesIdentity().getCelldesignerState()!=null){
        ...
        if(hm!=null){
          ...
          if(complexes.indexOf(complex_id)<0){
            ...
            physEntities.put(complex_id,compl);
            for(int j=0;j<ihm;j++){
                physicalEntityParticipant pep = createEntityParticipantForSpecies(sp.getId(),species,complexes_list,physEntities,sbml.getSbml(),biopax,csa_sp_id+"_"+(j+1));
    ...
    for(int i=0;i<reactions.size();i++)/*if(isRealReaction((Reaction)reactions.elementAt(i),sbml))*/{
      ...
      if(rtype.equals("TRANSPORT")){
        transport tr = biopax_DASH_level2_DOT_owlFactory.createtransport(biopax.namespaceString+r.getId(),biopax.biopaxmodel);
        conv = tr;
        inter = tr;
      }
      ...
      if(rtype.equals("CATALYSIS")||rtype.equals("UNKNOWN_CATALYSIS")||rtype.equals("TRANSCRIPTIONAL_ACTIVATION")||rtype.equals("TRANSLATIONAL_ACTIVATION")){
        catalysis cat = biopax_DASH_level2_DOT_owlFactory.createcatalysis(biopax.namespaceString+"c"+r.getId(),biopax.biopaxmodel);
        cat.setCONTROL_DASH_TYPE("ACTIVATION");
        physicalEntityParticipant pepr = createEntityParticipantForSpecies(r.getListOfReactants().getSpeciesReferenceArray(0).getSpecies(),species,complexes_list,physEntities,sbml.getSbml(),biopax,"c"+r.getId());
        cat.setCONTROLLER(pepr);
        physicalEntityParticipant pepp = createEntityParticipantForSpecies(r.getListOfProducts().getSpeciesReferenceArray(0).getSpecies(),species,complexes_list,physEntities,sbml.getSbml(),biopax,"c"+r.getId());
        cat.addPARTICIPANTS(pepp);
        ctrl = cat;
        inter = cat;

        biochemicalReaction bre = biopax_DASH_level2_DOT_owlFactory.createbiochemicalReaction(biopax.namespaceString+r.getId(),biopax.biopaxmodel);
        conv = bre;
      ...
      if(rtype.equals("HETERODIMER_ASSOCIATION")){
        complexAssembly cas = biopax_DASH_level2_DOT_owlFactory.createcomplexAssembly(biopax.namespaceString+r.getId(),biopax.biopaxmodel);
        conv = cas;
        inter = cas;
      }
      ...
      for(int j=0;j<r.getListOfModifiers().getModifierSpeciesReferenceArray().length;j++){
        ...
        physicalEntityParticipant pep = createEntityParticipantForSpecies(id,species,complexes_list,physEntities,sbml.getSbml(),biopax,r.getId());
        pep.setSTOICHIOMETRIC_DASH_COEFFICIENT(new Double(1));
        catalysis cat = biopax_DASH_level2_DOT_owlFactory.createcatalysis(biopax.namespaceString+r.getId()+"_c_"+id,biopax.biopaxmodel);
        path.addPATHWAY_DASH_COMPONENTS(cat);
        cat.setCONTROLLER(pep);
      ...
      /*if(inter!=null)if(r.getNotes()!=null)if(r.getNotes().getHtml()!=null){
        //System.out.println("COMMENT: "+r.getNotes().getHtml().getBody());
        inter.addCOMMENT(Utils.getValue(r.getNotes().getHtml().getBody()));
        Vector v = extractPubMedReferenceFromComment(Utils.getValue(r.getNotes().getHtml().getBody()));
      ...
      if(conv!=null){
        ...
        /*if(rtype.equals("CATALYSIS")||rtype.equals("UNKNOWN_CATALYSIS")||rtype.equals("TRANSCRIPTIONAL_ACTIVATION")||rtype.equals("TRANSLATIONAL_ACTIVATION")||
           rtype.equals("INHIBITION")||rtype.equals("UNKNOWN_INHIBITION")||rtype.equals("TRANSCRIPTIONAL_INHIBITION")||rtype.equals("TRANSLATIONAL_INHIBITION"))
             control = true;*/
        if(!iscontrol)
  ...
  */
public physicalEntityParticipant createEntityParticipantForSpecies(String id, Vector species, Vector complexes_list, HashMap physEntities, SbmlDocument.Sbml sbml, BioPAX biopax, String rId){
 ...
 //physicalEntityParticipant pep = biopax_DASH_level2_DOT_owlFactory.createphysicalEntityParticipant(biopax.namespaceString+sp.getId(),biopax.biopaxmodel);
 ...
 if((si!=null)&&(si.getCelldesignerState()!=null)&&(si.getCelldesignerState().getCelldesignerHomodimer()!=null)){
   //pep = biopax_DASH_level2_DOT_owlFactory.createsequenceParticipant(biopax.namespaceString+rId+"_"+Utils.correctName(spname),biopax.biopaxmodel);
   /*if(sp!=null)
     eid = sp.getId();
   else
     eid = sp_id;*/
   //eid = getComplexById(complexes_list,sp.getId()).getSpecies();
   //pep = biopax_DASH_level2_DOT_owlFactory.createphysicalEntityParticipant(biopax.namespaceString+rId+"_"+Utils.correctName(spname),biopax.biopaxmodel);
   pep = biopax_DASH_level2_DOT_owlFactory.createsequenceParticipant(biopax.namespaceString+rId+"_"+Utils.correctName(spname),biopax.biopaxmodel);
   //eid = getComplexById(complexes_list,sp.getId()).getSpecies();
   //eid = sp.getId();
 ...
 //if(sp.getSpeciesAnnotation().getCelldesigner_speciesIdentity().getCelldesigner_class().equals("DEGRADED"))
 //  System.out.println();
 if(pep==null){
 ...
 //System.out.println(sp.getCompartment());
 //System.out.println(((Compartment)compartmentHash.get(sp.getCompartment())).getName());
 openControlledVocabulary comp = null;
 if(sp!=null){
 ...
 pep.setCELLULAR_DASH_LOCATION(comp);

 if(sp!=null)
/* if(sp.getNotes()!=null)if(sp.getNotes().getHtml()!=null){
   //System.out.println("COMMENT: "+sp.getNotes().getHtml().getBody());
   pep.addCOMMENT(Utils.getValue(sp.getNotes().getHtml().getBody()));
   Vector v = extractPubMedReferenceFromComment(Utils.getValue(sp.getNotes().getHtml().getBody()));
   if(pep.getPHYSICAL_DASH_ENTITY()!=null)
     addPublicationXrefsToEntity(pep.getPHYSICAL_DASH_ENTITY(),v);
 }*/
 ...
 for(int j=0;j<modifs.length;j++){
   ...
   sequenceFeature sf = biopax_DASH_level2_DOT_owlFactory.createsequenceFeature(biopax.namespaceString+sp_id+""+s,biopax.biopaxmodel);
   String state = cm.getState().getStringValue();
   if(state.equals("unknown")) state+="_modification";
   sf.setFEATURE_DASH_TYPE((openControlledVocabulary)cellDesignerTerms.get(state));
   sf.setNAME(resname+"_"+stt);
  ...
  while(st.hasMoreTokens()){
    ...
    if(ss.toLowerCase().equals("pmid")){
      if(st.hasMoreTokens()){
        ...
        if(xref==null){
          xref = biopax_DASH_level2_DOT_owlFactory.createpublicationXref(biopax.namespaceString+"Pubmed_"+pubmedid,biopax.biopaxmodel);
          xref.setDB("Pubmed");
          xref.setID(pubmedid);
...
public void addPublicationXrefsToEntity(entity ent, Vector v){
  ...
  if(ent.getXREF()!=null){
  //while(ent.getXREF().hasNext()){
  //  present.add(ent.getXREF().next());
  //}
  }
...
public void createCellDesignerTerms(){
  try{
  openControlledVocabulary voc = null;
  String s = null;
  ...
  s = "CATALYSIS";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "UNKNOWN_CATALYSIS";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "INHIBITION";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "UNKNOWN_INHIBITION";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "TRANSCRIPTIONAL_ACTIVATION";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "TRANSCRIPTIONAL_INHIBITION";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "TRANSLATIONAL_ACTIVATION";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "TRANSLATIONAL_INHIBITION";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "STATE_TRANSITION";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "UNKNOWN_TRANSITION";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "HETERODIMER_ASSOCIATION";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "DISSOCIATION";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "TRANSPORT";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "KNOWN_TRANSITION_OMITTED";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "TRUNCATION";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  /*s = "phosphorylated";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "acetylated";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "ubiquitinated";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "methylated";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "hydroxylated";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "don't_care";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  s = "unknown";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);*/
  //s = "";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  //s = "";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  //s = "";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
  //s = "";  voc = biopax_DASH_level2_DOT_owlFactory.createopenControlledVocabulary(biopax.namespaceString+"cd3_"+s,biopax.biopaxmodel); voc.addTERM(s); cellDesignerTerms.put(s,voc);
